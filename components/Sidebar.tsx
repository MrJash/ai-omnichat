...
import { PlusCircle, MessageSquare, Trash2, Pencil } from './icons';
...
const Sidebar: React.FC<SidebarProps> = ({ sessions, activeSessionId, onNewChat, onSelectChat, onDeleteChat, onRenameChat, theme, onThemeChange }) => {
  ...
  return (
    <div className="w-64 bg-[var(--color-bg-primary)] flex flex-col p-2 border-r border-[var(--color-border)]">
      ...
      <div className="p-2 mt-auto text-xs text-gray-500">
        <ThemeSwitcher currentTheme={theme} onThemeChange={onThemeChange} />
        <p className="mt-4 text-center text-[var(--color-text-secondary)]/50">Powered by Gemini</p>
      </div>
    </div>
  );
};
